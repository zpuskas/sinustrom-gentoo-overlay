...
src_prepare() {
	...
	local cuda_supported_gcc="8.5 9.5 10 11 12 13 14"
...
src_install() {
	...
	if use nsight; then
		...
		"${ED}"/${cudadir}/${ncu_dir}/host/linux-desktop-glibc_2_11_3-x64/{libarrow.so,libparquet.so.1700.0.0} || die
		patchelf --set-rpath '$ORIGIN' \
		"${ED}"/${cudadir}/${nsys_dir}/host-linux-x64/{libarrow.so,libparquet.so.1700.0.0} || die
